...
def stochastic_oscillator_d(df, n):
    ...
    :param df: pandas.DataFrame
    :param n: period for calculation
    :return: pandas.DataFrame with %K and %D
    """
    ...
    # %K
    SOk = 100 * (df['Close'] - low_min) / (high_max - low_min)
    SOk.name = f'SO%K_{n}'
    
    # %D = simple moving average of %K
    SOd = SOk.rolling(window=3, min_periods=1).mean()
    SOd.name = f'SO%D_{n}'
...
def relative_strength_index(df, n):
    ...
    avg_gain = gain.ewm(span=n, min_periods=n).mean()
    avg_loss = loss.ewm(span=n, min_periods=n).mean()
...
def donchian_channel(df, n):
    """Calculate Donchian Channel for given pandas DataFrame.
    ...
    upper = df['High'].rolling(window=n, min_periods=1).max()
    # Lower band = rolling min of Low
    lower = df['Low'].rolling(window=n, min_periods=1).min()
    # Donchian channel = upper - lower
    donchian = upper - lower
    donchian.name = f'Donchian_{n}'
    df = df.join(donchian)
    return df
